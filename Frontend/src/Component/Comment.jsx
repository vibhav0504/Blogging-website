...
<div className="flex-shrink-0 mr-3"><img className="w-10 h-10 rounded-full bg-gray-200" src={user.profilePicture} alt={user.userName} /></div>
<div className="flex-1">
  <div className="flex items-center mb-1">
    <span className="font-bold mr-1 text-sm truncate">{user?`@${user.userName}`:"anonymous user"}</span>
    <span className="text-gray-500 texxt-sm">{moment(comment.createdAt).fromNow()}</span>
  </div>
