...
try {
  const res=await fetch(`/api/getallcomments?startIndex=${startIndex}`);
  console.log(res)
  const data=await res.json();
  if(res.ok){
...
return (
  ...
  <Table.Row >
   ...
   <Table.Cell onClick={()=>{setShowModal(true);setCommentIdToDelete(comment._id)}} className='text-red-400 hover:underline cursor-pointer'>
    Delete
   </Table.Cell>
